...
for(covname in c("SST.UPW.4.mon6to9.", "SST.2.5.mon6to9.", "Bakun.UPW.mon6to9.")){
  covtit <- getcovtit(covname)
...
tmp1=data.frame(
  Model=paste(addarrow, "2. $ln(N_t)$ = M + $\\beta V_t$"),
  Residual.df=round(a[["Resid. Df"]][i],digits=1), 
  MASE=round(mean(abs(residuals(get(mod))))/NE,digits=3), 
  Adj.R2=100*round(aa$r.sq, digits=3), 
  F=retF(a,i)$F, p.value = retF(a,i)$p, 
...
if(AIC(get(mod))<80) addarrow <- "$\\Rightarrow$"
tmp1=data.frame(
  Model=paste(addarrow, "3. $ln(N_t)$ = M + $s(V_t)$"),
  Residual.df=round(a[["Resid. Df"]][i],digits=1), 
  MASE=round(mean(abs(residuals(get(mod))))/NE,digits=3), 
  Adj.R2=100*round(aa$r.sq, digits=3), 
  F=retF(a,i)$F, p.value = retF(a,i)$p, 
