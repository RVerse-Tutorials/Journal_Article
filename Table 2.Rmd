...
aa = summary(get(mod))
tmp1=data.frame(
  Model="2. M1 $ln(N_t) = \\alpha + s(ln(N_{t-1})) + \\epsilon_t$", 
  Residual.df=round(a[["Resid. Df"]][i],digits=1), 
  MASE=round(mean(abs(residuals(get(mod))))/NE,digits=3), 
  Adj.R2=100*round(aa$r.sq, digits=2), 
...
aa = summary(get(mod))
tmp1=data.frame(
  Model="3a. $ln(N_t) = M1 + s_2(ln(N_{t-2}))$", 
  Residual.df=round(a[["Resid. Df"]][i],digits=1), 
  MASE=round(mean(abs(residuals(get(mod))))/NE,digits=3), 
  Adj.R2=100*round(aa$r.sq, digits=2), 
  F=round(a$F[i],digits=2), p.value=round(a[["Pr(>F)"]][i], digits=3), 
...
aa = summary(get(mod))
tmp1=data.frame(
  Model="$\\Rightarrow$ 3b. $ln(N_t) = M1 + s_2(ln(S_{t-2}))$", 
  Residual.df=round(a[["Resid. Df"]][i],digits=1), 
  MASE=round(mean(abs(residuals(get(mod))))/NE,digits=3), 
  Adj.R2=100*round(aa$r.sq, digits=2), 
  F=round(a$F[i],digits=2), p.value=round(a[["Pr(>F)"]][i], digits=3), 
