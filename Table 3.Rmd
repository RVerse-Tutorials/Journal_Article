...
covnames=c("SST.2.10.mon6to9.0", "SST.UPW.4.mon6to9.0","SST.2.10.3.yr.runsum.0")
...
tmp4 = data.frame(Model="$Z_{t}$ = 2.5-year average SST", Residual.df="", MASE="", Adj.R2="", F="", p.value="", AIC="", stringsAsFactors=FALSE)
...
m1 = gam(spawners0 ~ s(nspawners1, sp=0.6) + s(spawners2, sp=0.6), data=dat)
m2 = gam(spawners0 ~ s(nspawners1, sp=0.6) + s(spawners2, sp=0.6) + s(cov2, sp=0.6), data=dat)
a=anova(m1,m2, test="F")

i=2; mod=paste("m",i,sep="")
aa = summary(get(mod))
tmp1=data.frame(
  Model="$\\Rightarrow$ 2c. $ln(S_t) = M1 + s(Z_{t})$", 
  Residual.df=round(a[["Resid. Df"]][i],digits=1), 
  MASE=round(mean(abs(residuals(get(mod))))/NE,digits=3), 
  Adj.R2=100*round(aa$r.sq, digits=2), 
  F=round(a$F[i],digits=2), p.value=round(a[["Pr(>F)"]][i], digits=3), 
...
covnames=c("SST.2.10.mon3to5.0", "SST.UPW.4.mon6to9.0","SST.2.10.3.yr.runsum.0")
dat$cov0=dat[[covnames[1]]]
dat$cov1=dat[[covnames[2]]]
dat$cov2=dat[[covnames[3]]]
...
tmp4 = data.frame(Model="$Z_{t}$ = 2.5-year average SST", Residual.df="", MASE="", Adj.R2="", F="", p.value="", AIC="", stringsAsFactors=FALSE)
seltable=rbind(seltable,tmp1,tmp2,tmp3,tmp4)
...
aa = summary(get(mod))
tmp1=data.frame(
  Model="$\\Rightarrow$ 2c. $ln(N_t) = M1 + s(Z_{t})$", 
  Residual.df=round(a[["Resid. Df"]][i],digits=1), 
  MASE=round(mean(abs(residuals(get(mod))))/NE,digits=3), 
  Adj.R2=100*round(aa$r.sq, digits=2), 
  F=round(a$F[i],digits=2), p.value=round(a[["Pr(>F)"]][i], digits=3), 
