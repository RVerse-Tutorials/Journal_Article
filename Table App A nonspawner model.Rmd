...
tmp1=data.frame(
  Model="1. $ln(N_t)$ = $\\alpha_t$ + $\\epsilon_t$", 
...
err <- mod$model$data[1,]-modloo$pred
tmp1=data.frame(
  Model="2. $ln(N_t)$ = $\\alpha_t$ + $\\beta_t t$ + $\\epsilon_t$", 
  Residual.df=mod$samp.size-attr(MARSS:::logLik.marssMLE(mod), "df"), 
  MASE=round(mean(abs(MARSS:::residuals.marssMLE(mod, silent=TRUE)$model.residuals), na.rm=TRUE)/NE,digits=3), 
  Adj.R2="", 
  F="", p.value="", 
...
err <- mod$model$data[1,]-modloo$pred
tmp1=data.frame(
  Model="3. $ln(N_t)$ = $\\alpha_t$ + $\\beta_t ln(S_{t-1})$ + $\\epsilon_t$", 
  Residual.df=mod$samp.size-attr(MARSS:::logLik.marssMLE(mod), "df"), 
  MASE=round(mean(abs(MARSS:::residuals.marssMLE(mod, silent=TRUE)$model.residuals), na.rm=TRUE)/NE,digits=3), 
  Adj.R2="", 
  F="", p.value="", 
...
err <- mod$model$data[1,]-modloo$pred
tmp1=data.frame(
  Model="3. $ln(N_t)$ = $\\alpha_t$ + $\\beta_t ln(N_{t-1})$ + $\\epsilon_t$", 
  Residual.df=mod$samp.size-attr(MARSS:::logLik.marssMLE(mod), "df"), 
  MASE=round(mean(abs(MARSS:::residuals.marssMLE(mod, silent=TRUE)$model.residuals), na.rm=TRUE)/NE,digits=3), 
  Adj.R2="", 
  F="", p.value="", 
...
thecap=paste("Model selection tests of time-dependency the $N_t$ model using non-linear or time-varying linear responses instead of time-constant linear responses as in Table",ref("tabappA:nonspawner-model-time-a", pre="A"), "See Table",ref("tabappA:nonspawner-model-time-a", pre="A"), "for an explanation of the parameters and model set-up.")
