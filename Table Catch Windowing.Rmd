...
for(t in yr1:yr2){
  ...
  mod = list()
  mod[[1]] = lm(Catch0 ~ -1 + offset(Catch1), data=m0$model)
  mod[[2]]  = lm(Catch0 ~ offset(Catch1), data=m0$model)
  mod[[3]] = gam(Catch0 ~ Catch1, data=m0$model)
  mod[[4]]  = gam(Catch0 ~ Catch1 + Catch2, data=m0$model)
  mod[[5]]  = gam(Catch0 ~ s(Catch1, sp=0.6), data=m0$model)
  mod[[6]]  = gam(Catch0 ~ s(Catch1, sp=0.6)+s(Catch2, sp=0.6), data=m0$model)
}
#naive model
mod="mne"
aa = summary(get(mod))
