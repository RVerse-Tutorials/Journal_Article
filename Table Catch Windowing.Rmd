...
window = 20
respdat = fullrespdat[which(fullrespdat$Year==1956):which(fullrespdat$Year==2016),]
yr1 = fullrespdata$Year[1]
yr2 = max(fullrespdata$Year)-window
...
#naive model error
NE=mean(abs(respdat$Catch0-respdat$Catch1), na.rm=TRUE)
...
for(t in y1:y2){
  w1 = t; w2 = t+window-1
  dat=respdat[which(respdat$Year==w1):which(respdat$Year==w2),]
  m0 = gam(Catch0 ~ Catch1 + Catch2, data=dat)
  mne = lm(Catch0 ~ -1 + offset(Catch1), data=m0$model)
  ar1 = lm(Catch0 ~ offset(Catch1), data=m0$model)
  l1 = gam(Catch0 ~ Catch1, data=m0$model)
  l2 = gam(Catch0 ~ Catch1 + Catch2, data=m0$model)
  g1 = gam(Catch0 ~ s(Catch1, sp=0.6), data=m0$model)
  g2 = gam(Catch0 ~ s(Catch1, sp=0.6)+s(Catch2, sp=0.6), data=m0$model)
}
#naive model
mod="mne"
aa = summary(get(mod))
