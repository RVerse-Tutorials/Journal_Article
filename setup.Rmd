...
respdat=respdat[respdat$Year<=max(seio_covariates_mon$Year),]
  ...
  covnames=c(
    "log.CHL.2","log.CHL.3","log.CHL.4","log.CHL.5",
    "log.CHL.2","log.CHL.3","log.CHL.4","log.CHL.5",
    "log.CHL.2","log.CHL.3","log.CHL.4","log.CHL.5",
    "log.CHL.2","log.CHL.3","log.CHL.4","log.CHL.5",
    "log.CHL.2","log.CHL.3","log.CHL.4","log.CHL.5",
    "log.CHL.2","log.CHL.3","log.CHL.4","log.CHL.5",
    "log.CHL.2","log.CHL.3","log.CHL.4","log.CHL.5",
    "log.CHL.7","log.CHL.8","log.CHL.9","log.CHL.10",
    "log.CHL.7","log.CHL.8","log.CHL.9","log.CHL.10",
    "log.CHL.7","log.CHL.8","log.CHL.9","log.CHL.10",
    "log.CHL.7","log.CHL.8","log.CHL.9","log.CHL.10",
    "log.CHL.7","log.CHL.8","log.CHL.9","log.CHL.10",
...
for(j in c("mon9to10.0", "mon6to9.0", "mon1to12.0", "mon3to5.0",  "mon10to12.0",  "mon7to9.0")){
  varname=c(paste0("SST.",c(2:5),".",j))
  tmp=apply(respdat[,varname],1,mean,na.rm=TRUE)
  varname=paste0("SST.2.10.",j)
  respdat[[varname]]=tmp
}
for(j in c("mon9to10.0", "mon6to9.0", "mon1to12.0", "mon3to5.0",  "mon10to12.0",  "mon7to9.0")){
  varname=c(paste0("log.CHL.",c(2:5,7:10),".",j))
  tmp=apply(respdat[,varname],1,mean,na.rm=TRUE)
  varname=paste0("log.CHL.2.10.",j)
  respdat[[varname]]=tmp
}
...
tmp=(respdat[,"SST.2.10.mon1to12.0"]*12 - respdat[,"SST.2.10.mon10to12.0"]*3 - respdat[,"SST.2.10.mon7to9.0"]*3) +
    respdat[,"SST.2.10.mon1to12.1"]*12 +
    respdat[,"SST.2.10.mon1to12.2"]*12
varname="SST.2.10.3.yr.runsum.0"
respdat[[varname]]=tmp/(12+12+6)
#make 2 year runsum of SST
tmp=(respdat[,"SST.2.10.mon1to12.0"]*12 +
    respdat[,"SST.2.10.mon1to12.1"]*12 + 
    respdat[,"SST.2.10.mon1to12.2"]*12)
varname="SST.2.10.2.yr.runsum.0"
respdat[[varname]]=tmp/(12+12+12)
#create the lag 1
for(i in c("SST.2.10.3.yr.runsum.0","SST.2.10.2.yr.runsum.0")){
  name=paste(str_sub(i, 1, str_length(i)-1),"1",sep="")
  respdat[[name]]=c(NA,respdat[[i]][1:(dim(respdat)[1]-1)])
  name=paste(str_sub(i, 1, str_length(i)-1),"2",sep="")
  respdat[[name]] = c(NA,NA,respdat[[i]][1:(dim(respdat)[1]-2)])
...
covnames = c("precip.gpcp.kerala.mon6to7.", "precip.gpcp.kerala.mon4to5.", 
             ...
             "SST.2.10.3.yr.runsum.", "ONI.mon1to12.",
             "DMI.mon9to11.")
```

```{r message=FALSE,warning=FALSE, echo=FALSE}
#set up to use consistent data
dat=fullrespdat[,c("Year","spawners0", "nspawners1", "nspawners2", "spawners0", "spawners1", "spawners2", paste(covnames, "0", sep=""), paste(covnames, "1", sep=""))]
dat=na.omit(dat)
dat.spawners=dat
...
covnames = c("log.CHL.4.mon1to3.", "log.CHL.4.mon7to9.", "log.CHL.4.mon10to12.",
             ...
             "SST.2.10.3.yr.runsum.", "ONI.mon1to12.")
#set up to use consistent data
dat=fullrespdat[,c("Year", "spawners0", "nspawners0","nspawners2",
                   "nspawners1", "spawners2", "spawners1",
...
covnames = c("precip.gpcp.kerala.mon6to7.", "precip.gpcp.kerala.mon4to5.", 
             "SST.4.mon1to12.", "SST.4.mon6to9.", 
             "SST.UPW.4.mon1to3.", "SST.UPW.4.mon6to9.", 
             "SST.UPW.4.mon10to12.", "SST.UPW.4.mon4to6.",
             "SST.2.10.mon6to9.", "SST.2.10.mon9to10.", 
             "SST.2.10.mon1to12.", 
             "SST.2.10.mon3to5.", "SST.2.10.mon10to12.",
             "SST.2.10.3.yr.runsum.", "ONI.mon1to12.",
             "SST.2.10.2.yr.runsum.")
#set up to use consistent data
