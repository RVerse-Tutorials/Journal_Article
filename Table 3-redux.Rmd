...
# Space
spacetmp = data.frame(Model="\\kern 1em",  Residual.df="\\kern 1em",  MASE="\\kern 1em", MAE="\\kern 1em", RMSE="\\kern 1em", LOOCV="\\kern 1em", Adj.R2="", F="\\kern 1em", p.value="\\kern 1em", AIC="", stringsAsFactors=FALSE)
```
...
covnames=c("SST.2.5.mon6to9.0", "Bakun.UPW.mon6to9.0","SST.2.10.3.yr.runsum.0")
...
tmp2$Model = "$V_t$ = Jun-Sep ns-SST current season"
tmp3$Model = "$W_{t}$ = Jun-Sep Bakun-UPW current season"
tmp4$Model = "$Z_{t}$ = 2.5-year ave r-SST"
...
tmp1 <- modfun(mod, Model)
seltable=rbind(seltable,tmp1)

i=2; mod=paste("m",i,sep="")
Model="$\\Rightarrow  ln(S_t) = M0 + s(V_t)$"
tmp1 <- covfun(mod, Model)
seltable=rbind(seltable,tmp1)
...
covnames=c("SST.2.10.mon3to5.0", "SST.2.5.mon6to9.0","SST.2.10.3.yr.runsum.0","DMI.mon9to11.1")
dat$cov0=dat[[covnames[1]]]
dat$cov1=dat[[covnames[2]]]
dat$cov2=dat[[covnames[3]]]
...
tmp2$Model = "$U_t$ = Mar-May r-SST current season"
tmp3$Model = "$V_{t}$ = Jun-Sep ns-SST current season"
tmp4$Model = "$Z_{t}$ = 2.5-year ave r-SST"
tmp5$Model = "$X_{t-1}$ = fall DMI prior season"
