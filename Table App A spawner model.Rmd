...
thecap=paste("Model selection tests of time-dependency the $S_t$ model using F-tests of nested models fit to log landings data.  $S_t$ is the catch during the spawning period (Jul-Sep) of season $t$ (Jul-Jun). $N_t$ is the catch during the non-spawning period (Oct-Jun). $S_{t-1}$ and $N_{t-1}$ are the catch during the prior sardine season during and after the spawning period respectively. $S_{t-2}$ and $N_{t-2}$ are the same for two seasons prior.  Test A uses catch during the spawning period as the explanatory variable.  Test B uses catch during the non-spawning period as the explanatory variable.  Test C uses both. For Test C, the nestedness is lines 1-3 and lines 1-2 and 4.")
fullcap=paste("Table ", ref("tabappA:spawner-model-time-a", pre="A"), ". ", thecap, sep="")
...
thecap=paste("Model selection tests of time-dependency the $S_t$ model using non-linear responses instead of linear responses as in Table",ref("tabappA:spawner-model-time-a", pre="A"), "See Table",ref("tabappA:spawner-model-time-a", pre="A"), "for an explanation of the parameters and model set-up.")
fullcap=paste("Table ", ref("tabappA:spawner-model-time-b", pre="A"), ". ", thecap, sep="")
kable(seltable, align="lccccc", caption=fullcap)
```
...
thecap=paste("Model selection tests of time-dependency for the $S_t$ model using 1956-2015 data.  See Table",ref("tabappA:spawner-model-time-a", pre="A"),"for definitions.")
fullcap=paste("Table ", ref("tabappA:spawner-model-time-c", pre="A"), ". ", thecap, sep="")
kable(seltable, align="lccccc", caption=fullcap)
```
