...
DD1 = data.frame(
  Life="Age 2+",
  Hyp="DD1.  $S_t$ is dominated by age 2+ fish, thus abundance of the 1-yr and 2-yr ages in the prior season (Oct-Mar catch) should be correlated with the abundance of mature fish this year.",
  Response="$S_t$",
  Cov="$N_{t-1}$",
...
DD2 = data.frame(
  Life="Age 1-2",
  Hyp="DD2.  Abundance of 1-yr and 2-yr fish should be correlated with strength of the cohorts from the previous two seasons.",
...
DD3 = data.frame(
  Life="Age 2+",
  Hyp="DD3.  Because age 2 fish appear in the post-monsoon catch, we expect the post-monsoon catch (dominated by age 1 and 2) in the previous season to be correlated with the post-monsoon catch in the current season.",
  Response="$N_t$",
  Cov="$N_{t-1}$",
  stringsAsFactors=FALSE)
...
HA2 = data.frame(
  Life="All ages",
  Hyp="A2. The changes brought about by the El Niño/Southern Oscillation (ENSO) cycle have a variety of effects on environmental parameters (precipitation, SST, thermal fronts, wind) which impacts spawning and early survival with a 1-year lag. ",
  Response="$N_t$ and $S_t$",
  Cov="ONI in year t-1",
  stringsAsFactors=FALSE)
