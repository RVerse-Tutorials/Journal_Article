...
```{r setup-figures, include=FALSE, message=FALSE, warning=FALSE}
...
```{r fig-study-area, out.width = "400px", fig.align='center', echo=FALSE, fig.cap="Close up of the Kerala district (see inset of India for overall location).  The boxes show the regions over which the satellite data were averaged.  The Kerala district is marked in grey and the oil sardine catch from this region is being modelled.",message=FALSE,warning=FALSE}
...
```{r fig-currents, out.width = "500px", fig.align='center', echo=FALSE, fig.cap="Key currents and circulation patterns in the Indian Ocean that affect winter and summer monsoon associated upwelling.", message=FALSE, warning=FALSE}
...
```{r fig-chl-2016, out.width = "400px", fig.align='center', echo=FALSE, fig.cap="Average Chlorophyll-a bloom intensity in 2016 by quarter. Red is high density chlorophyll-a.  Black is missing data due to cloud cover. Data from NOAA.", message=FALSE, warning=FALSE}
...
```{r sst.mon, echo=FALSE,fig.cap="Monthly sea surface temperature 1981-2017 off the Kerala coast.", fig.width=8,fig.height=8,message=FALSE,warning=FALSE}
### FIGURE SST
oldpar=par(mfrow=c(1,2), mar=c(2,5,2,0))
i=4
  covname=paste("SST.",i,sep="")
dat=seio_covariates_mon[[covname]]
datmon=seio_covariates_mon$Month
datts=ts(dat,start=1956, frequency=12)
plot(datts,ylab=paste("SST Box",i),xlab="",bty="n",ylim=c(24,31),xlim=c(2000,2005), yaxs="i", xaxs="i")
...
catchmeans=tapply(dat, datmon, mean, na.rm=TRUE)
names(catchmeans)=1:12
barplot(catchmeans,ylab="",ylim=c(24,31),col="grey",offset=0,xpd=FALSE, axis.lty=1, bty="l")
