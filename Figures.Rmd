...
for(start in yrs){
  ...
  legend(length(dat2)-11.5,ymin+3,c("SST near-shore","SST off-shore","Chl-a"), lwd=c(2,1,10),col=c("red","black","green"),bty="n",cex=1,seg.len=1)
...
dat$cov0=dat[["SST.2.10.3.yr.runsum.0"]]
loc=which(dat.spawners$Year %in% c(1986, 1991, 1994, 2013))
...
plot(m0$model[,"spawners0"],m0$fitted.values, xlim=c(xmin,xmax), ylim=c(xmin,xmax),
     xlab="Log Jul-Sep catch", ylab="Predicted log catch\nwithout covariates")
lines(c(xmin,xmax),c(xmin,xmax-jig))
legend("topleft",paste("R2 =",round(100*summary(m0)$r.sq,digits=1),"\nWithout covariates"), bty="n")
legend("topright","A", bty="n")
text(m0$model[loc,"spawners0"], m0$fitted.values[loc], pos=4, offset=1)
...
legend("topleft",paste("R2 =",round(100*summary(m1)$r.sq,digits=1),"\nWith multi-year SST covariate"), bty="n")
legend("topright","B", bty="n")
text(m0$model[loc,"spawners0"], m0$fitted.values[loc], pos=4, offset=1)
...
plot(m0$model[,"nspawners0"],m0$fitted.values, xlim=c(xmin,xmax), ylim=c(xmin,xmax),
     xlab="Log Oct-Mar catch", ylab="Predicted log catch\nwithout covariates")
