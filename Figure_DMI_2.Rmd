...
dat2$nspawners0[dat2$Year==1994]=NA
dat2$cat0=MARSS::zscore(dat2$nspawners0)
dat2$cat1=MARSS::zscore(dat2$nspawners1)
...
fit.cat <- MARSS(dat2$cat0, model=modlist, silent=TRUE, method="BFGS", inits=inits.list)
...
A=matrix(0)
R=matrix("r")
Q=matrix(list(0),2,2); diag(Q)=list(0.01,"q")
Q=diag(0.01,2)
inits.list = list(x0=matrix(c(0), nrow=2))
modlist = list(Z=Zt, Q=Q, U="zero", A=A, R=R)
fit.dmi <- MARSS(dat2$cat0, model=modlist, silent=TRUE, method="BFGS", inits=inits.list)
...
inits.list = list(x0=matrix(c(0), nrow=2))
modlist = list(Z=Zt, Q=Q, U="zero", A=A, R=R)
fit.chl <- MARSS(dat2$cat0, model=modlist, silent=TRUE, method="BFGS", inits=inits.list)
...
inits.list = list(x0=matrix(c(0), nrow=2))
modlist = list(Z=Zt, Q=Q, U="zero", A=A, R=R)
fit.precip <- MARSS(dat2$cat0, model=modlist, silent=TRUE, method="BFGS", inits=inits.list)
...
R=matrix("r")
Q=diag(1,2) #fix so doesn't go to zero
Q=diag(c(0.01,q.dmi, q.precip))
Q=diag(c(0.01,0.01, 0.01))

inits.list = list(x0=matrix(c(0), nrow=3))
modlist = list(Z=Zt, Q=Q, U=matrix(0, nrow=3), A=A, R=R)
fit.dmi.precip <- MARSS(dat2$cat0, model=modlist, silent=TRUE, method="BFGS", inits=inits.list)
```
...
val=c()
for(i in ii){
  val<- rbind(val,c(sqrt(mean(err[1,i:(i+win-1)]^2, trim=trim)), 
                    sqrt(mean(err[2, i:(i+win-1)]^2, trim=trim))))
}
return(val)
}
...
val=c()
for(i in ii){
  val<- rbind(val,c(mean(abs(err[1,i:(i+win-1)]), trim=trim), 
                    mean(abs(err[2, i:(i+win-1)]), trim=trim)))
}
return(val)
}
```
...
plot(dat2$Year, fit.dmi$states[2,], type="l", lwd=2, ylab="Time-varying effect size", xlab="", col="red", ylim=c(-1.5,.5))
lines(dat2$Year, fit.precip$states[2,], col="grey", lwd=2)
abline(h=0, lty=2)
legend("bottomright", bty="n", 
       legend=c("DMI","Precipitation"),
       col=c("red", "grey"), lwd=c(2,2))
legend("topleft", legend="a", bty="n")

#RMSE plot
err.dmi <- rbind(null=MARSSkfss(fit.cat)$Innov, dmi=MARSSkfss(fit.dmi)$Innov)
err.dmi.precip <- rbind(null=MARSSkfss(fit.cat)$Innov, 
               dmi=MARSSkfss(fit.dmi.precip)$Innov)
err.precip <- rbind(null=MARSSkfss(fit.cat)$Innov, 
               dmi=MARSSkfss(fit.precip)$Innov)
err.chl <- rbind(null=dat2$resid, 
               dmi=(fitted(fit.chl, type="ytt1")$.fitted-dat2$resid))
win=10
...
axis(side=2, at=log(c(1, 1.10, 1.25, 1.5, 1.75, 2, 5)), labels=c(0, 10, 25, 50, 75, 100, 150))
...
axis(side=2, at=log(c(1, 1.10, 1.25, 1.5, 1.75, 5)), labels=c(0, 10, 25, 50, 75, 100))
lines(dat2$Year[ii+win], val[,2], col="red", lwd=2)
